...
class QuantumSelfAttention(nn.Module):
    ...
    def __init__(self, n_qubits, n_layers, device, embed_dim, max_seq_len=128):
        ...
        # Interference / phase parameter per token position
        # Shape: (1, max_seq_len), broadcasted across batch
    ...
    def forward(self, X):
        ...
        for b in range(batch_size):
            ...
            K = self.q_kernel.compute_kernel_matrix(quantum_input[b], self.theta)  # (seq_len, seq_len)

            # Compute interference term (phase differences)
            phi_b = self.phi[:, :seq_len]  # (1, seq_len)
            phi_diff = phi_b.T - phi_b  # (seq_len, seq_len)
            K_interfered = K + torch.cos(phi_diff)  # add interference
        ...
        attn_outputs = torch.stack(attn_outputs)  # (batch_size, seq_len, embed_dim)
        attn_matrices = torch.stack(attn_matrices)  # (batch_size, seq_len, seq_len)
