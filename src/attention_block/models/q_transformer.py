...
#         # Pooling layer (mean over tokens)
...
#         x = self.token_embed(x) + self.pos_embed[:, :seq_len, :]  # (batch_size, seq_len, embed_dim)

#         attn_weights_all = []

#         # Pass through stacked quantum transformer blocks
...
class QTransformer(nn.Module):
    def __init__(self, vocab_size, embed_dim, n_qubits, n_layers, device, q_device,
                 num_blocks=2, num_classes=2, ff_hidden_dim=128, max_seq_len=128, dropout=0.1):
        super().__init__()
        self.embed_dim = embed_dim
        self.max_seq_len = max_seq_len
        self.device = device
        ...
        self.blocks = nn.ModuleList([
            QuantumTransformerBlock(
                ...
                n_qubits=n_qubits,
                n_layers=n_layers,
                device=self.q_device,  # pass PennyLane device
                ff_hidden_dim=ff_hidden_dim,
                dropout=dropout,
                max_seq_len=max_seq_len
            )
