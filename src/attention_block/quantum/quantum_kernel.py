...
#             Args:
#                 x (torch.Tensor): Input feature vector (length n_qubits)
#                 weights (torch.Tensor): Ansatz parameters (n_layers, n_qubits, 2)
...
#         expvals = []
#         for i in range(batch_size):
#             expval = self.expval_circuit(X[i], weights)  # list
#             expval = torch.tensor(expval, dtype=X.dtype)  # convert to tensor
#             expvals.append(expval)
