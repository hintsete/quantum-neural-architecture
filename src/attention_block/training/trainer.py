...
class Trainer:
    """
    Generic Trainer for training PyTorch models (classical or quantum transformers).
    ...
    def train_epoch(self, dataloader: DataLoader) -> dict:
        ...
        for batch in tqdm(dataloader, desc="Training", leave=False):
            ...
            outputs = self.model(inputs)  # For transformers, this returns logits
            if isinstance(outputs, tuple):  # If model returns (logits, attn)
                logits = outputs[0]
            else:
                logits = outputs
    ...
    def fit(self, train_loader: DataLoader, val_loader: DataLoader,
        ...
        """
        Full training loop with optional validation per epoch.
